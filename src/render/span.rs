...
//!
//! Three things enforce this rather than asking for it:
//!
...
//! 2. The only ways to make a span take a source string and a byte range, so
//!    "text is a substring of the input" is a fact about the constructor
//!    rather than a convention a later renderer has to remember.
...
pub enum SpanKind {
    ...
    /// original: `codepoint` and `name` are only what the screen shows in its
    /// place.
    Chip {
        codepoint: char,
        name: &'static str,
...
impl Span {
    ...
    ///
    /// Taking the source and a range rather than an owned string is what makes
    /// "the text of a span is a substring of the input" true by construction.
    ///
    /// Prefer [`SpanBuilder`], which additionally guarantees that a whole
    /// sequence of spans tiles its source exactly once.
    ///
    /// # Panics
    ///
    /// If `range` is out of bounds or does not fall on character boundaries.
    pub fn new(source: &str, range: Range<usize>, kind: SpanKind) -> Self {
    ...
    /// real text, and it is used only by the UI — never by [`unrender`].
...
///
/// [`SpanBuilder`] cannot produce a sequence that fails this. It is public so
/// that a renderer which builds spans some other way, and the tests for one,
/// can check themselves at the point the mistake is made rather than leaving
/// it for the property test to find later.
...
impl Spans {
    ...
    /// Split the span at `index` in two at absolute source offset `at`. Both
    /// halves keep the original kind; `break_before` stays with the left half,
    /// which is the one that still starts where the original did. The right
    /// half lands at `index + 1`.
    ///
...
mod tests {
    ...
    fn sample() -> (String, Spans) {
        ...
        builder.break_next();
        builder.push_to(5, SpanKind::Chip {
            codepoint: '\u{202E}',
            name: "[RLO]",
        });
